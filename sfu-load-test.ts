...
async function request(label: string, index: number, url: string, body: unknown): Promise<Result> {
...
async function scenarioA(): Promise<void> {
  ...
  const header =
    "CONCURRENCY |  total | success | deadlock | timeout |  avg |  P50 |  P95 |  P99 |  max";
  ...
  for (const concurrency of CONFIG.SCENARIO_A_LEVELS) {
    const tasks = Array.from({ length: concurrency }, (_, i) =>
      request(`trigger-safe c=${concurrency}`, i, `${CONFIG.BASE_URL}/deadlock/trigger-safe`, {
  ...
  console.log("\n  [観察ポイント]");
  console.log(
    "  - CONCURRENCY 増加に伴い P95/P99 が線形に伸びているか？（SFU キューイングの証拠）",
  );
...
async function scenarioB(): Promise<void> {
  ...
  const unsafeTasks = Array.from({ length: concurrency }, (_, i) =>
    request("trigger-unsafe", i, `${CONFIG.BASE_URL}/deadlock/trigger`, {
      roomId: CONFIG.ROOM_ID_HOT,
    }),
  );
  ...
  const safeTasks = Array.from({ length: concurrency }, (_, i) =>
    request("trigger-safe", i, `${CONFIG.BASE_URL}/deadlock/trigger-safe`, {
      roomId: CONFIG.ROOM_ID_HOT,
    }),
  );
...
async function scenarioC(): Promise<void> {
  ...
  const baselineTasks = Array.from({ length: coldCount }, (_, i) =>
    request("baseline-cold", i, `${CONFIG.BASE_URL}/deadlock/trigger-safe`, {
      roomId: CONFIG.ROOM_ID_COLD,
    }),
  );
  const baselineResults = await Promise.all(baselineTasks);
  const bs = stats(baselineResults);
  console.log(
    `     avg=${bs.avg}ms  P50=${bs.p50}ms  P95=${bs.p95}ms  P99=${bs.p99}ms  max=${bs.max}ms`,
  );
  ...
  const coldTasks = Array.from({ length: coldCount }, (_, i) =>
    request("contaminated-cold", i, `${CONFIG.BASE_URL}/deadlock/trigger-safe`, {
      roomId: CONFIG.ROOM_ID_COLD,
    }),
  );
  ...
  console.log(
    `     avg=${cs.avg}ms  P50=${cs.p50}ms  P95=${cs.p95}ms  P99=${cs.p99}ms  max=${cs.max}ms`,
  );
  ...
  const recoveryTasks = Array.from({ length: coldCount }, (_, i) =>
    request("recovery-cold", i, `${CONFIG.BASE_URL}/deadlock/trigger-safe`, {
      roomId: CONFIG.ROOM_ID_COLD,
    }),
  );
  ...
  console.log(
    `     avg=${rs.avg}ms  P50=${rs.p50}ms  P95=${rs.p95}ms  P99=${rs.p99}ms  max=${rs.max}ms`,
  );
  ...
  console.log(hr("-", 64));
  console.log(
    `${"ベースライン（hot 負荷なし）".padEnd(30)} | ${String(bs.avg).padStart(4)} | ${String(bs.p50).padStart(4)} | ${String(bs.p95).padStart(4)} | ${String(bs.p99).padStart(4)} | ${String(bs.max).padStart(4)}`,
  );
  console.log(
    `${"hot 高競合中（同時発射）".padEnd(30)} | ${String(cs.avg).padStart(4)} | ${String(cs.p50).padStart(4)} | ${String(cs.p95).padStart(4)} | ${String(cs.p99).padStart(4)} | ${String(cs.max).padStart(4)}`,
  );
  console.log(
    `${"回復後".padEnd(30)} | ${String(rs.avg).padStart(4)} | ${String(rs.p50).padStart(4)} | ${String(rs.p95).padStart(4)} | ${String(rs.p99).padStart(4)} | ${String(rs.max).padStart(4)}`,
  );
  ...
  console.log("  [観察ポイント]");
  console.log(
    "  - 「hot 高競合中」の cold avg が「ベースライン」より有意に増加すればプール汚染の証拠",
  );
...
async function main() {
  ...
  console.log(
    `BASE_URL: ${CONFIG.BASE_URL}  ROOM_HOT: ${CONFIG.ROOM_ID_HOT}  ROOM_COLD: ${CONFIG.ROOM_ID_COLD}`,
  );
  console.log(hr("=", 64));
